...
local function makeAuto(name)
    if not shotguns.Guns[name] then
        local func 
        local gs = getsenv(Backpack[name].GunClientHandler)
        
        for i,v in pairs(gs) do 
            if type(v) == "function" and getinfo(v).name == "Fire" then 
                func = v
                setconstant(func,112,"Auto")
            end
        end
    end
end
local function modGun(nameOfGun,newCurrentammo,newMaxammo,newFireRate)
    local func

    if Backpack[nameOfGun].GunClientHandler and not shotguns.Guns[nameOfGun] then
        local gs = getsenv(Backpack[nameOfGun].GunClientHandler)
        ...
        for i,v in pairs(gs) do
            if type(v) == "function" and getinfo(v).name == "Fire" then
                ...
                setupvalue(func,4,newMaxammo)
                setupvalue(func,15,newCurrentammo)
                setupvalue(func,17,newFireRate)
            end
        end
        else
            for i,v in pairs(gs) do
                if type(v) == "function" and getinfo(v).name == "Fire" then
                    ...
                    -- 3 = FireRate
                    setupvalue(func,4,newMaxammo)
                    setupvalue(func,14,newCurrentammo)
                    setupvalue(func,16,newFireRate)
                end
            end
    end
end 
...
do -- Example UI
    local Window = library:AddWindow("SITE PHOENIX GUN MODDER", {
        main_color = Color3.fromRGB(41, 74, 122),
    ...
    local Tab = Window:AddTab("GUN MODS")
    
    
    do -- Elements
        Tab:AddLabel("SITE PHOENIX GUN MODDER")
        
        Tab:AddTextBox("Gun Name", function(text)
            gunName = text
        ...
        Tab:AddButton("Mod gun", function()
            if gunName then
                modGun(gunName,currentAmmo,maxAmmo,fireRate)

            end
        end)
        Tab:AddButton("Make Auto",function()
            if gunName and gunName ~= shotguns.Guns[gunName] then
                makeAuto(gunName)
        ...
        local CURRENTAMMO = Tab:AddSlider("CURRENTAMMO:", function(x)
            currentAmmo = x 
    
        end, { -- (options are optional)
            ["min"] = 0, -- Default: 0
            ["max"] = 50000, -- Default: 100
            ["readonly"] = false, -- Default: false
        })
        CURRENTAMMO:Set(50)
        local MAXAMMO = Tab:AddSlider("MAXAMMO",function(x)
            maxAmmo = x 
        end, {
            ["min"] = 0,
            ["max"] = 50000,
        ...
        Tab:AddTextBox("FIRE RATE:",function(text)
            fireRate = tonumber(text)
        end,{
            ["clear"] = false,
        })
